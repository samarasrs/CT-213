...
def policy_evaluation(grid_world, initial_value, policy, num_iterations=10000, epsilon=1.0e-5):
    ...
    value_anterior = np.copy(value)
    # Todo: implement policy evaluation.

    cont = 0
    dif = np.zeros([dimensions[0], dimensions[1]])
    maior_dif = inf

    while cont < num_iterations and maior_dif >= epsilon:
        cont += 1
        for i in range(dimensions[0]):
            for j in range(dimensions[1]):
                current_state = (i, j)
                ...
                for action in range(NUM_ACTIONS):
                    ...
                    for next_state in grid_world.get_valid_sucessors(current_state, action):
                        transition_prob = grid_world.transition_probability(current_state, action, next_state)
                        value[i][j] += grid_world.gamma * pi * transition_prob * value_anterior[next_state[0], next_state[1]]
                dif[i][j] = value[i][j] - value_anterior[i][j]
        maior_dif = np.max(np.absolute(dif))
        value_anterior = np.copy(value)
        ...
        for i in range(dimensions[0]):
            for j in range(dimensions[1]):
                ...
                for action in range(NUM_ACTIONS):
                    ...
                    for next_state in grid_world.get_valid_sucessors(current_state, action):
                        transition_prob = grid_world.transition_probability(current_state, action, next_state)
                        value[i][j] += grid_world.gamma * transition_prob * value_anterior[next_state[0], next_state[1]]
                    if value[i][j] > max_value:
                        max_value = value[i][j]
                dif[i][j] = value_anterior[i][j] - max_value
                value[i][j] = max_value
        maior_dif = np.max(np.abs(dif))
        value_anterior = np.copy(value)
