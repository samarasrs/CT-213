...
def make_detector_network(img_cols, img_rows):
    ...
    # Layer 8
    layer = Conv2D(filters=64, kernel_size=(3, 3), strides=(1, 1), padding='same', name='conv_8', use_bias=False)(layer)
    layer = BatchNormalization(name='norm_8')(layer)
    layer = LeakyReLU(alpha=0.1, name='leaky_relu_8')(layer)


    # Layer 7B
    skip_connection = Conv2D(filters=128, kernel_size=(3, 3), strides=(1, 1), padding='same', name='conv_skip', use_bias=False)(
        skip_connection)
