...
plt.xlabel('Epoch')
plt.ylabel('Cost')
plt.title('Cost Function Convergence (XOR)')
plt.grid()
plt.savefig('cost_function_convergence(XOR).' + fig_format, format=fig_format)
...
plt.xlabel('x1')
plt.ylabel('x2')
plt.title('Neural Network Classification (XOR)')
plt.savefig('neural_net_classification(XOR).' + fig_format, format=fig_format)
plt.show()
